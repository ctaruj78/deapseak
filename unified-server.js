...
app.get('/api/users/profile', authenticateToken, async (req, res) => {
    try {
        ...
        const user = await db.collection('users').findOne(
            { _id: userId },
            { projection: { password: 0 } } // Не віддаємо пароль
        );
        ...
        console.log('✅ Профіль завантажено:', user.email);
        res.json(user); // Повертаємо об'єкт користувача
    } catch (error) {
...
// PUT /api/users/profile - оновлення профілю поточного користувача
app.put('/api/users/profile', authenticateToken, async (req, res) => {
    try {
        ...
        const allowedFields = ['firstName', 'lastName', 'phone', 'company', 'address', 'city', 'country', 'bio', 'avatar'];
        const updateData = {};
        ...
        console.log('✅ Профіль оновлено:', result.value.email);
...
app.post('/api/users/change-password', authenticateToken, async (req, res) => {
    try {
        ...
        if (!currentPassword || !newPassword) {
            return res.status(400).json({
                success: false,
                message: 'Вкажіть поточний та новий пароль'
            });
        }
...
// POST /api/users/avatar - завантаження аватара (заглушка)
app.post('/api/users/avatar', authenticateToken, async (req, res) => {
    try {
        // TODO: Додати multer для завантаження файлів
        // Поки що просто приймаємо URL аватара
        const { avatar } = req.body;
        
        if (!avatar) {
            return res.status(400).json({
                success: false,
                message: 'Аватар не вказано'
            });
        }
        ...
        console.log('✅ Аватар оновлено для користувача:', req.user.email);
        res.json({
            success: true,
        ...
        res.status(500).json({
            success: false,
            message: 'Помилка оновлення аватара'
        });
    }
});

// ========================================
// КІНЕЦЬ PROFILE API ENDPOINTS
// ========================================
